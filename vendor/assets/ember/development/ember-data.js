...
// Fetched on: 2015-06-02T15:08:46Z
(function() {
    "use strict";
    ...
    */
    function ember$data$lib$system$store$serializer$response$$normalizeResponseHelper(serializer, store, modelClass, payload, id, requestType) {
      ...
      normalizedSerializerPayload = ember$data$lib$system$store$serializer$response$$_normalizeSerializerPayload(modelClass, serializerPayload);
    ...
    function ember$data$lib$system$store$serializer$response$$_normalizeSerializerPayload(modelClass, payload) {
      ...
      if (payload) {
        if (Ember.isArray(payload)) {
          normalizedPayload.data = ember$data$lib$system$store$serializer$response$$map(payload, function (payload) {
            return ember$data$lib$system$store$serializer$response$$_normalizeSerializerPayloadItem(modelClass, payload);
          });
        } else {
          normalizedPayload.data = ember$data$lib$system$store$serializer$response$$_normalizeSerializerPayloadItem(modelClass, payload);
        }
      }
    ...
    function ember$data$lib$system$store$serializer$response$$pushPayloadData(store, payload) {
      ...
      if (payload && payload.data) {
        if (Ember.isArray(payload.data)) {
          result = ember$data$lib$system$store$serializer$response$$_pushResourceObjects(store, payload.data);
        } else {
    ...
    */
    function ember$data$lib$system$store$serializer$response$$pushPayloadIncluded(store, payload) {
      ...
      if (payload && payload.included && Ember.isArray(payload.included)) {
        result = ember$data$lib$system$store$serializer$response$$_pushResourceObjects(store, payload.included);
      }
      return result;
    }
    ...
    */
    function ember$data$lib$system$store$serializer$response$$_pushResourceObjects(store, resourceObjects) {
      return ember$data$lib$system$store$serializer$response$$map(resourceObjects, function (resourceObject) {
        return ember$data$lib$system$store$serializer$response$$_pushResourceObject(store, resourceObject);
      });
    }
    ...
    */
    ...
    var ember$data$lib$serializers$rest2$serializer$$forEach = Ember.ArrayPolyfills.forEach;
    //var map = Ember.ArrayPolyfills.map;

    function ember$data$lib$serializers$rest2$serializer$$coerceId(id) {
      return id == null ? null : id + "";
    }

    var ember$data$lib$serializers$rest2$serializer$$REST2Serializer = ember$data$lib$serializers$rest$serializer$$default.extend({

      normalize: function (typeClass, hash, prop) {
        return {
          data: ember$data$lib$system$store$serializer$response$$_normalizeSerializerPayloadItem(typeClass, this._super(typeClass, hash, prop)),
          included: []
        };
      },
      ...
      normalizeResponse: function (store, primaryTypeClass, payload, id, requestType) {
        ...
        for (var prop in payload) {
          ...
          if (isPrimary && Ember.typeOf(value) !== "array") {
            ...
            var _normalize = this.normalize(primaryTypeClass, value, prop);
          ...
          if (isSingle) {
            /*jshint loopfunc:true*/
            ember$data$lib$serializers$rest2$serializer$$forEach.call(data, function (resource) {

              var isFirstCreatedRecord = isPrimary && !id && !document.data;
              var isUpdatedRecord = isPrimary && ember$data$lib$serializers$rest2$serializer$$coerceId(resource.id) === id;
            ...
            if (isPrimary) {
              ...
              (_document$included2 = document.included).push.apply(_document$included2, data);
            }
          }

          (_document$included3 = document.included).push.apply(_document$included3, included);
        }
      ...
      pushPayload: function (store, rawPayload) {
        ...
        for (var prop in payload) {
          ...
          /*jshint loopfunc:true*/
          ember$data$lib$serializers$rest2$serializer$$forEach.call(Ember.makeArray(payload[prop]), function (hash) {
            var _document$included4;
            ...
            (_document$included4 = document.included).push.apply(_document$included4, included);
          }, this);
        }

        ember$data$lib$system$store$serializer$response$$pushPayload(store, document);
      },

      normalizeArray: function (store, typeName, arrayHash, prop) {
        var data = [];
        var included = [];
        var array = this._super(store, typeName, arrayHash, prop);

        array.forEach(function (item) {
          data.push(item.data);
          included.push.apply(included, item.included);
        });
    ...
    var ember$data$lib$serializers$rest2$serializer$$default = ember$data$lib$serializers$rest2$serializer$$REST2Serializer;

    var activemodel$adapter$lib$system$active$model2$serializer$$forEach = Ember.EnumerableUtils.forEach;
    var activemodel$adapter$lib$system$active$model2$serializer$$camelize = Ember.String.camelize;
    var activemodel$adapter$lib$system$active$model2$serializer$$classify = Ember.String.classify;
    var activemodel$adapter$lib$system$active$model2$serializer$$decamelize = Ember.String.decamelize;
    var activemodel$adapter$lib$system$active$model2$serializer$$underscore = Ember.String.underscore;

    var activemodel$adapter$lib$system$active$model2$serializer$$ActiveModel2Serializer = ember$data$lib$serializers$rest2$serializer$$default.extend({
      ...
      keyForAttribute: function (attr) {
        return activemodel$adapter$lib$system$active$model2$serializer$$decamelize(attr);
      },

      keyForRelationship: function (relationshipTypeKey, kind) {
        var key = activemodel$adapter$lib$system$active$model2$serializer$$decamelize(relationshipTypeKey);
        if (kind === "belongsTo") {
          return key + "_id";
        } else if (kind === "hasMany") {
          return ember$inflector$lib$lib$system$string$$singularize(key) + "_ids";
      ...
      payloadKeyFromModelName: function (modelName) {
        return activemodel$adapter$lib$system$active$model2$serializer$$underscore(activemodel$adapter$lib$system$active$model2$serializer$$decamelize(modelName));
      },

      serializePolymorphicType: function (snapshot, json, relationship) {
        var key = relationship.key;
        var belongsTo = snapshot.belongsTo(key);
        var jsonKey = activemodel$adapter$lib$system$active$model2$serializer$$underscore(key + "_type");

        if (Ember.isNone(belongsTo)) {
          json[jsonKey] = null;
        } else {
          json[jsonKey] = activemodel$adapter$lib$system$active$model2$serializer$$classify(belongsTo.modelName).replace(/(\/)([a-z])/g, function (match, separator, chr) {
            return match.toUpperCase();
          }).replace("/", "::");
        }
      },
      ...
      normalizeLinks: function (data) {
        if (data.links) {
          ...
          for (var link in links) {
            var camelizedLink = activemodel$adapter$lib$system$active$model2$serializer$$camelize(link);
          ...
          typeClass.eachRelationship(function (key, relationship) {
            ...
            if (relationship.options.polymorphic) {
              ...
              if (payload && payload.type) {
                ...
                var self = this;
                activemodel$adapter$lib$system$active$model2$serializer$$forEach(payload, function (single) {
                  single.type = self.modelNameFromPayloadKey(single.type);
                });
              }
            } else {
      ...
      modelNameFromPayloadKey: function (key) {
        var convertedFromRubyModule = activemodel$adapter$lib$system$active$model2$serializer$$camelize(ember$inflector$lib$lib$system$string$$singularize(key)).replace(/(^|\:)([A-Z])/g, function (match, separator, chr) {
          return match.toLowerCase();
        }).replace("::", "/");
        return ember$data$lib$system$normalize$model$name$$default(convertedFromRubyModule);
      }
    ...
    var activemodel$adapter$lib$system$active$model2$serializer$$default = activemodel$adapter$lib$system$active$model2$serializer$$ActiveModel2Serializer;
    ...
    var ember$data$lib$core$$DS = Ember.Namespace.create({
      VERSION: '1.0.0-beta.19+canary.c2f44b0b5c'
    });
    ...
    */
    ...
    function ember$data$lib$system$store$finders$$_findHasMany(adapter, store, record, link, relationship) {
      ...
      return promise.then(function (adapterPayload) {
        return store._adapterRun(function () {
          var payload = ember$data$lib$system$store$serializer$response$$normalizeResponseHelper(serializer, store, relationship.type, adapterPayload, null, "findHasMany");
    ...
    function ember$data$lib$system$store$finders$$_findBelongsTo(adapter, store, record, link, relationship) {
      ...
      return promise.then(function (adapterPayload) {
        return store._adapterRun(function () {
          var payload = ember$data$lib$system$store$serializer$response$$normalizeResponseHelper(serializer, store, relationship.type, adapterPayload, null, "findBelongsTo");
      ...
      */
      push: function (modelName, data) {
        ...
        if (Ember.isArray(internalModel)) {
          internalModel = ember$data$lib$system$store$$map(internalModel, function (item) {
      ...
      _pushInternalModel: function (modelName, data) {
        if (Ember.typeOf(modelName) === "object" && Ember.typeOf(data) === "undefined") {
          return ember$data$lib$system$store$serializer$response$$pushPayload(this, data);
        }
    ...
    */
    ...
    var ember$data$lib$serializers$embedded$records2$mixin$$get = Ember.get;
    var ember$data$lib$serializers$embedded$records2$mixin$$set = Ember.set;

    var ember$data$lib$serializers$embedded$records2$mixin$$EmbeddedRecords2Mixin = Ember.Mixin.create(ember$data$lib$serializers$embedded$records$mixin$$default, {

      normalize: function (typeClass, hash, prop) {
        var normalizedHash = this._super.apply(this, arguments);
        return ember$data$lib$serializers$embedded$records2$mixin$$extractEmbeddedRecords(this, this.store, typeClass, normalizedHash);
      }
    });

    function ember$data$lib$serializers$embedded$records2$mixin$$extractEmbeddedRecords(serializer, store, typeClass, partial) {
      typeClass.eachRelationship(function (key, relationship) {
        if (serializer.hasDeserializeRecordsOption(key)) {
          var embeddedTypeClass = store.modelFor(relationship.type.modelName);
          if (relationship.kind === "hasMany") {
            ember$data$lib$serializers$embedded$records2$mixin$$extractEmbeddedHasMany(store, key, embeddedTypeClass, partial, relationship);
          }
          if (relationship.kind === "belongsTo") {
            ember$data$lib$serializers$embedded$records2$mixin$$extractEmbeddedBelongsTo(store, key, embeddedTypeClass, partial, relationship);
          }
        }
      });
      return partial;
    ...
    function ember$data$lib$serializers$embedded$records2$mixin$$extractEmbeddedHasMany(store, key, typeClass, hash, relationshipMeta) {
      var relationshipHash = ember$data$lib$serializers$embedded$records2$mixin$$get(hash, "data.relationships." + key + ".data");
      if (!relationshipHash) {
        return;
      }
      ...
      var hasMany = relationshipHash.map(function (item) {
        ...
        var embeddedTypeClass = typeClass;
        ...
        var _embeddedSerializer$normalize = embeddedSerializer.normalize(embeddedTypeClass, item, null);

        var data = _embeddedSerializer$normalize.data;
        var included = _embeddedSerializer$normalize.included;
      ...
      ember$data$lib$serializers$embedded$records2$mixin$$set(hash, "data.relationships." + key, relationship);
    }

    function ember$data$lib$serializers$embedded$records2$mixin$$extractEmbeddedBelongsTo(store, key, typeClass, hash, relationshipMeta) {
      var _hash$included2;
      ...
      var embeddedTypeClass = typeClass;
      if (relationshipMeta.options.polymorphic) {
        var modelName = relationshipHash.type;
        embeddedTypeClass = store.modelFor(modelName);
      }

      var embeddedSerializer = store.serializerFor(embeddedTypeClass.modelName);

      var _embeddedSerializer$normalize2 = embeddedSerializer.normalize(embeddedTypeClass, relationshipHash, null);

      var data = _embeddedSerializer$normalize2.data;
      var included = _embeddedSerializer$normalize2.included;

      hash.included.push(data);
      (_hash$included2 = hash.included).push.apply(_hash$included2, included);

      var belongsTo = { id: data.id, type: embeddedTypeClass.modelName };

      var relationship = { data: belongsTo };
      ember$data$lib$serializers$embedded$records2$mixin$$set(hash, "data.relationships." + key, relationship);
    }

    var ember$data$lib$serializers$embedded$records2$mixin$$default = ember$data$lib$serializers$embedded$records2$mixin$$EmbeddedRecords2Mixin;
    ...
    */
    ...
    ember$data$lib$core$$default.REST2Serializer = ember$data$lib$serializers$rest2$serializer$$default;
    ...
    ember$data$lib$core$$default.ActiveModel2Serializer = activemodel$adapter$lib$system$active$model2$serializer$$default;

    ember$data$lib$core$$default.EmbeddedRecordsMixin = ember$data$lib$serializers$embedded$records$mixin$$default;
    ember$data$lib$core$$default.EmbeddedRecords2Mixin = ember$data$lib$serializers$embedded$records2$mixin$$default;
